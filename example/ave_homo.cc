...
Eigen::VectorXd homo(double ka, int nx, int ny, int cx, int cy, int xx,
                     int yy, const Eigen::VectorXd& x0, int n_iter = 1,
  ...
  double node_dens = 400 / (w_rve + h_rve);  // 800 nodes.

  Boundary<AP, 4> b{node_dens,
                    {{-d / 2 * nx + d * xx, -d / 2 * ny + d * yy + h_rve},
                     {-d / 2 * nx + d * xx + w_rve, -d / 2 * ny + d * yy}},
                    &m};
...
Eigen::VectorXd ave_homo(double ka, int nx, int ny, int cx, int cy, int xx,
                         int yy, int ax, int ay,
                         const Eigen::VectorXd& x0 = Eigen::Vector4d::Ones(),
                         int n_iter = 1, int n_hop = 1, double d = 0.2) {
  Eigen::VectorXd x(4), xx0(x0);
  x.setZero();

  for (int i = 0; i < ax; i++)
    for (int j = 0; j < ay; j++) {
      xx0 = homo(ka, nx, ny, cx, cy, xx + i * d, yy + j * d, xx0, n_iter,
                 n_hop);
      x += xx0;
    }
  return x / ax / ay;
